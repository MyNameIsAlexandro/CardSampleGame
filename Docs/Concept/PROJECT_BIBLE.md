# PROJECT\_BIBLE v1.0 — ECHO: Legends of the Veil
...
- **Кооп:** в v1.0 **нет**, но архитектура проектируется так, чтобы 2–4 игрока добавить без ломки системы
...
- в v1.0 герой меняется через колоду/бремя;
- в v1.1+ добавим эволюцию/аспекты (state machine) и «пороки».
...
## 11) Scope v1.0 (чтобы не утонуть)
