...
### 5.2 Единый конфликт (Disposition Combat)

> **Подробная спецификация:** [disposition-combat-design.md](../plans/2026-02-18-disposition-combat-design.md)

Каждая встреча — это **Disposition Track**: единая шкала от -100 (уничтожен) до +100 (подчинён).

**Три способа воздействия (drag-drop):**

- **Strike** (drag на врага) — сдвигает к -100 (уничтожение)
- **Influence** (drag на алтарь) — сдвигает к +100 (подчинение)
- **Sacrifice** (drag на костёр) — сжигает карту навсегда, даёт энергию

**Два способа победы с последствиями:**

- **Уничтожение (-100):** враг покидает мир навсегда. Loot, ресурсы. Resonance сдвигается к тьме.
- **Подчинение (+100):** враг остаётся в мире. Информация, союзник. Resonance сдвигается к свету.

**Тактическая глубина:**

- Карты универсальны — каждая может быть Strike, Influence или Sacrifice
- Momentum система: повторение одного типа усиливает его, но делает переключение дороже
- Враг адаптируется: при приближении к краю шкалы переключает режим (Survival / Desperation)
- Resonance зона меняет эффективность действий между локациями

**Это решает сразу 3 задачи:**

- «мирный путь» и «боевой путь» — части одной системы через одну шкалу;
- каждый бой — решение под давлением, не оптимизация формулы;
- балансируется контент‑паками (affinity matrix, enemy decks, resonance modifiers) без новых механик.
...
### Фаза B — Столкновение (Disposition Combat)

- враг телеграфирует намерение (intent) + показывает текущий режим;
- игрок перетаскивает карты в зоны: Strike (на врага), Influence (на алтарь), Sacrifice (на костёр);
- Disposition Track сдвигается к -100 (уничтожение) или +100 (подчинение);
- Momentum, Fate keywords и Resonance меняют правила на лету.
...
- Disposition Combat encounter для всех врагов;
...
- Все конфликты решаются только уничтожением (нет пути подчинения).
...
**Disposition Combat:**
Разбойник: можно уничтожить (Strike к -100) или подчинить (Influence к +100). Исход меняет резонанс, будущие ветки и доступность врага в мире.
