...
**Версия:** 3.2 (Legacy cleanup: archive MVP, single Resonance axis, authoring note)
**Дата:** 29 января 2026
...
| **Авторинг → Runtime** | JSON (авторинг) → binary .pack (runtime). См. CHANGELOG v2.2.0 |
...
- Моральные выборы (сдвиг WorldResonance / PlayerAffinity)
...
**Визуальное состояние:** Нормальные цвета, солнечный свет, живая природа. «Человеческая» естественность — тепло, быт, дым печей, живые звуки.
...
**Визуальное состояние:** Туман, сумерки, тревожная атмосфера. На подступах — шёпоты и тени Нави. Звуки приглушены, природа настороженна. NPC нервничают, говорят тише.
...
**Визуальное состояние:** Плотный туман/тьма, искажения, мертвая природа. Мир темнеет, звук становится «глуше», появляются шёпоты/дальние голоса. NPC пугаются/избегают. Меньше живых деталей, больше теней и холода.
...
#### **1. Укреплять якорь (strengthenAnchor)** ⚒️
- **Светлый/нейтральный герой (balance ≥ 30):**
  - **Стоимость:** Вера (из `anchor.strengthenCost`)
  - **Эффект:** +Integrity (из `anchor.strengthenAmount`), alignment НЕ меняется
- **Тёмный герой (balance < 30, alignment = Nav):**
  - **Стоимость:** HP (из `anchor.darkStrengthenCostHP`, default 3)
  - **Эффект:** +Integrity, alignment сдвигается к Dark (light→neutral→dark)
- **Последствие:** Улучшение состояния региона
- **Реализация:** `TwilightGameEngine.executeStrengthenAnchor()`
...
#### **3. Осквернять якорь (defileAnchor)** 💀
- **Доступ:** Только тёмный герой (balance < 30, alignment = Nav)
- **Стоимость:** HP (из `anchor.defileCostHP`, default 5)
- **Эффект:** Alignment якоря → Dark (мгновенно), Integrity НЕ меняется
- **Ограничение:** Нельзя осквернить уже тёмный якорь
- **Реализация:** `TwilightGameEngine.executeDefileAnchor()`
...
### 6.5 Матрица состояний региона

Регион имеет два независимых измерения:

| | **Light** | **Neutral** | **Dark** |
|---|---|---|---|
| **Stable** | Безопасное святилище | Обычный стабильный | Контролируемая тьма |
| **Borderland** | Ослабленный свет | Пограничье | Наступающая тьма |
| **Breach** | Падший свет | Прорыв Нави | Бездна |

- **Alignment региона** = alignment его якоря (или neutral если якоря нет)
- Alignment хранится в `EngineAnchorState.alignment` и сохраняется в save
- `EngineRegionState.alignment` — computed property от якоря
...
**Описание:** Сражение с врагами из Нави или враждебными NPC. Разрешается через **Disposition Combat** (единая шкала -100…+100).
...
**Механика (Disposition Combat + Intent):**
- Враг раскрывает **Intent** (намерение) перед ходом игрока: атака, защита, спецприём
- Игрок выбирает путь победы: **Destroy (disposition → -100)** или **Subjugate (disposition → +100)**
- Drag-and-drop: Strike (→ врага), Influence (→ алтарь), Sacrifice (→ костёр)
- Исход определяет сдвиг резонанса и доступные награды/ветки

**Награды:**
- Destroy: карты врагов, трофеи, вера (больше лута, резонанс → Навь)
- Subjugate: знания, репутация, благословения (меньше лута, резонанс → Правь)
- Артефакты, продвижение квеста
...
**Последствия:**
- Сдвиг резонанса (shiftResonance / shiftAffinity, см. [COMBAT_DIPLOMACY_SPEC.md §4](./COMBAT_DIPLOMACY_SPEC.md#4-влияние-резонанса))
...
**Механика (Fate Test):**
```
Исследовать → Fate Test (Stat + FateCard + Effort >= Difficulty)
  ↓
Успех: Награда (артефакт, карты, информация)
Провал: Наказание (проклятие, враг, потеря ресурсов)

Effort: игрок может сбросить карты с руки, каждая даёт +1 к проверке.
Это создаёт решение: потратить карты сейчас (гарантировать успех) или рискнуть.
```
...
- WorldResonance ≤ -40 (склонность к Нави)
...
- WorldResonance в зоне Яви (-39..+39)
...
**Последствие:** Сдвиг WorldResonance / PlayerAffinity
...
- Горный дух (ритуал, выбор Навь/Правь)
...
- Выбор: Правь (восстановить якоря), Навь (использовать силу Нави), Явь (найти равновесие)
- Флаг: `chosen_path_prav/nav/yav = true`
...
**PlayerAffinity (Склонность героя):** -100 до +100
- **<-40:** Герой склоняется к Нави
- **-39..+39:** Герой в равновесии (Явь)
- **>+40:** Герой следует путём Прави

**Как изменяется:**
- → Правь: Восстановление якорей, помощь людям, Subjugate в бою (disposition → +100)
- → Навь: Оскверненние якорей, сделки с Навью, Destroy в бою (disposition → -100)

**Влияние:**
- Цена карт (affinity: nav/prav) в разных зонах резонанса
- Match Bonus и Synergy/Dissonance для Fate Cards
- Определяет доступные финалы, реакции NPC, уникальные события
...
✅ **Система резонанса (WorldResonance / PlayerAffinity):**
- Определяет доступные карты и их стоимость
- Влияет на финал и Synergy/Dissonance

---

### 13.2 Структуры данных

> **Актуальные структуры:** См. [ENGINE_ARCHITECTURE.md](../Technical/ENGINE_ARCHITECTURE.md) и [SPEC_CAMPAIGN_PACK.md](../Specs/SPEC_CAMPAIGN_PACK.md)
>
> Ниже — **концептуальная модель** для понимания дизайна. Реализация использует `id: String` (stable IDs для save/load), данные загружаются из Content Pack.

#### Концептуальные структуры

| Сущность | Ключевые поля | Примечания |
|----------|---------------|------------|
| **Region** | `id: String`, `state`, `anchorId?`, `neighborIds` | Состояние: stable/borderland/breach |
| **Anchor** | `id: String`, `integrity: 0-100`, `affinity: nav/yav/prav` | Определяет состояние региона |
| **Event** | `id: String`, `type`, `choices[]`, `consequences` | Data-driven из JSON |
| **Quest** | `id: String`, `stages[]`, `rewards` | Main (5 актов) или Side |
| **WorldRuntimeState** | `worldTension`, `worldResonance`, `flags` | Engine-managed state |

#### Ключевые принципы:
- **String IDs** — стабильные для save/load совместимости (не UUID)
- **Engine-first** — все изменения через `TwilightGameEngine.performAction()`
- **Pack-driven** — структуры определяются в Content Pack JSON
- **Deterministic** — все random через seeded WorldRNG
...
3. WorldTension и WorldResonance
...
> **Каноническая реализация:** См. [ENGINE_ARCHITECTURE.md](../Technical/ENGINE_ARCHITECTURE.md)
> Время продвигается через `TwilightGameEngine.performAction()` → внутренний TimeEngine.

При начале каждого дня Engine выполняет:

1. **Увеличить счётчик дней** — `daysPassed += 1`
2. **Каждые 3 дня:**
   - Увеличить WorldTension: `+3 + (daysPassed / 10)` (эскалация)
   - Проверить деградацию регионов
3. **Проверить триггеры** квестов и мировых сдвигов

> Все числовые значения (интервал, формула эскалации) берутся из Balance Pack.
...
2. Если регион имеет якорь с integrity > 0:
   ...
   - проверка: если WorldRNG.checkProbability(шанс) → деградация отменена
...
- **Disposition Combat инструмент:** карты играются в трёх режимах — Strike (disposition -), Influence (disposition +), Sacrifice (heal + exhaust)

**Примеры использования карт:**
- «Удар Мечом» → Strike: высокий base_power для уничтожения
- «Заговор Зубов» → Influence: высокий base_power для подчинения
- «Божественный Удар» → Универсальная: эффективна и в Strike, и в Influence
- «Целебный Отвар» → Sacrifice: сильный heal при exhaust
...
| Роль     | Функция в кампании      | Баланс     | Редкость   | Track Target |
|----------|-------------------------|------------|------------|--------------|
| Sustain  | Выживание, восстановление | Свет       | Common/Uncommon | — (Heal) |
| Control  | Защита мира, якоря       | Свет       | Rare/Epic  | Spirit |
| Power    | Быстрый прогресс, сила   | Тьма       | Uncommon/Rare | Body |
| Utility  | Гибкость, манипуляция    | Нейтральный | Common/Uncommon | Both |

> **Track Target** определяет, по какой шкале врага карта наносит урон: Body (Physical), Spirit (Spiritual), Both или — (не боевая).

---

## 23. НАВЬ / ПРАВЬ КАК ЭКОНОМИЧЕСКИЙ ВЫБОР

> Навь/Правь — это не "зло/добро", а разные стратегии прохождения.
> Числовая ось: WorldResonance (-100..+100), PlayerAffinity (-100..+100).
> См. [COMBAT_DIPLOMACY_SPEC.md §4](./COMBAT_DIPLOMACY_SPEC.md#4-влияние-резонанса).
...
### 23.4 Влияние резонанса на стоимость карт

> **Канонический расчёт:** См. [COMBAT_DIPLOMACY_SPEC.md §4.2](./COMBAT_DIPLOMACY_SPEC.md#42-система-аффинити-карт)

Стоимость карт модифицируется через систему аффинити (`card.affinity: nav | prav | nil`) и текущую зону WorldResonance:
...
> **Примечание:** Старая функция `adjustedCost(balance: Int)` с шкалой 0–100 удалена.
> Все расчёты используют WorldResonance (-100..+100).

---

## 23.5 Система аналитики выборов (оффлайн)

> **Требование из PROJECT_BIBLE (v1.0 MUST)**

Игра логирует ключевые решения и сдвиги в локальный журнал для балансировки будущих паков:

| Что логируется | Зачем |
|----------------|-------|
| **Destroy vs Subjugate** по каждому столкновению | Баланс двух путей |
| **Сдвиги резонанса** (amount + source) | Темп смещения мира |
| **Выборы в событиях** (choiceId + consequences) | Популярность веток |
| **Смена стратегии в бою** (Strike-streak vs Influence-streak, switch penalty) | Глубина тактики |

**Реализация:**
- Локальное хранение (JSON/SQLite)
- Экспорт по желанию игрока
- В v1.0 — без серверной части
...
> **ВАЖНО:** Любой рандом должен быть **воспроизводимым** после Save/Load.
> Используется `WorldRNG.shared` (seeded RNG), а не `randomElements()`.

```swift
func generateMarket(engine: TwilightGameEngine, currentRegion: Region) -> [Card] {
    var market: [Card] = []

    // 1. Глобальный пул (3 карты) — deterministic selection via seeded RNG
...
> **Gate test:** `SaveLoadRoundTripTests` проверяет, что рынок идентичен после Save/Load.
...
- Выборы формируют стиль игры (сдвиг WorldResonance / PlayerAffinity)
...
| 3 | Стоимость карт зависит от WorldResonance + card.affinity (см. §23.4) | 📋 Планируется |
...
```
1. Создать две колоды: Prav-focused и Nav-focused
2. Пройти один и тот же акт обеими колодами
3. Убедиться:
   - Разные события доступны
   - Разные цены в рынке (affinity modifiers)
   - Разные последствия решений
   - Разный прогресс по времени
4. Проверить:
   - Prav-колода медленнее, но стабильнее (Subjugate через Influence)
   - Nav-колода быстрее, но WorldTension выше (Destroy через Strike)
```
...
|--------------|------------------|---------------|-------|
| <40 | Правь (>+40) | saved_all_anchors | 🌟 Золотой рассвет |
| <60 | Явь (-39..+39) | — | ⚖️ Хрупкий мир |
| <60 | Навь (<-40) | used_navi_core | 🌑 Тёмное спасение |
| 60-80 | любая | — | ⚠️ Пиррова победа |
| >80 | — | — | 💀 Падение Яви |
...
### 32.5 Подсчёт Deck Path (PlayerAffinity)
...
| **Тональные изменения** | При Nav-пути (глубокая Навь) мир описывается мрачнее |
...
```
1. Пройти Акт I, выбирая только Prav-решения (Subjugate, восстановление)
2. Пройти Акт I, выбирая только Nav-решения (Destroy, оскверненние)
3. Убедиться:
   - Разные NPC доступны
...
## 19. ИСТОРИЯ РЕАЛИЗАЦИИ

> **Архив:** Исторические данные о реализации MVP и ранних версий перенесены в
> [Archive/ECD_IMPLEMENTATION_HISTORY.md](../Archive/ECD_IMPLEMENTATION_HISTORY.md).
> Этот документ содержит только актуальные механики, алгоритмы, формулы и JSON-структуры.

---

**Статус документа:** ✅ Core Design Complete + Act I Spec
**Версия:** 3.2
**Дата:** 29 января 2026
