...
**Описание:** Сражение с врагами из Нави или враждебными NPC. Разрешается через **Unified Encounter** (Dual Track: Body + Spirit).
...
**Механика (Dual Track + Intent):**
- Враг раскрывает **Intent** (намерение) перед ходом игрока: атака, защита, спецприём
- Игрок выбирает путь победы: **Kill (Body→0)** или **Pacify (Spirit→0)**
- Проверки через **Fate Test** (Stat + Fate Card + Effort >= Difficulty)
- Исход определяет сдвиг резонанса и доступные награды/ветки

**Награды:**
- Kill: карты врагов, трофеи, вера (больше лута, резонанс → Навь)
- Pacify: знания, репутация, благословения (меньше лута, резонанс → Правь)
...
**Как изменяется:**
- → Правь: Восстановление якорей, помощь людям, Pacify в бою
- → Навь: Оскверненние якорей, сделки с Навью, Kill в бою
...
- **Dual Track инструмент:** карты делятся на **Physical** (бьют по Body) и **Spiritual** (бьют по Spirit/Resolve)

**Примеры Track-специализации карт:**
- «Удар Мечом» → Body damage (Physical)
- «Заговор Зубов» → Spirit damage (Spiritual)
- «Божественный Удар» → Body + Spirit damage (Both)
- «Целебный Отвар» → Heal (не привязан к Track)
...
|----------------|-------|
| **Kill vs Pacify** по каждому столкновению | Баланс двух путей |
| **Сдвиги резонанса** (amount + source) | Темп смещения мира |
| **Выборы в событиях** (choiceId + consequences) | Популярность веток |
| **Смена стратегии в бою** (начал Kill → перешёл Pacify и наоборот) | Глубина тактики |
...
4. Проверить:
   - Prav-колода медленнее, но стабильнее (Pacify-путь)
   - Nav-колода быстрее, но WorldTension выше (Kill-путь)
```
...
```
1. Пройти Акт I, выбирая только Prav-решения (Pacify, восстановление)
2. Пройти Акт I, выбирая только Nav-решения (Kill, оскверненние)
3. Убедиться:
   - Разные NPC доступны
