...
### Pillar 6. Единый конфликт — Disposition Combat

> **Подробная спецификация:** См. [COMBAT_DIPLOMACY_SPEC.md](./COMBAT_DIPLOMACY_SPEC.md)
> **Полный дизайн:** [`docs/plans/2026-02-18-disposition-combat-design.md`](../../docs/plans/2026-02-18-disposition-combat-design.md)

- Единая шкала **Disposition** (-100…+100): уничтожение ←→ подчинение
- **Strike** (drag→враг): сдвиг к -100, уничтожение — быстрее, но резонанс → Навь
- **Influence** (drag→алтарь): сдвиг к +100, подчинение — сложнее, но Правь усиливается
- **Sacrifice** (drag→костёр): exhaust карту навсегда → heal + enemy buff
- Каждый бой — **моральное решение**: способ победы определяет последствия
- **Momentum system**: streak bonus при последовательных действиях, switch penalty при смене
- **Hard cap**: ни одно действие не сдвигает шкалу больше чем на 25 единиц
...
### 9.2 Механика боя (v4.0 — Disposition Combat)

> **Версия 4.0:** Конфликты разрешаются через **Disposition Track** — единую шкалу -100…+100. Игрок выбирает путь победы: уничтожение (-100) или подчинение (+100).

#### Disposition Track — единая шкала:

> **Подробная спецификация:** См. [COMBAT_DIPLOMACY_SPEC.md](./COMBAT_DIPLOMACY_SPEC.md)
> **Полный дизайн:** [`docs/plans/2026-02-18-disposition-combat-design.md`](../../docs/plans/2026-02-18-disposition-combat-design.md)

| Край | Описание | Последствия |
|------|----------|-------------|
| **-100 (Уничтожен)** | Враг покидает мир навсегда | Loot, ресурсы; resonance → Навь |
| **+100 (Подчинён)** | Враг остаётся в мире | Информация, союзник; resonance → Правь |

Стартовое значение определяется **Affinity Matrix** (heroWorld × enemyType + situationModifier).

#### 3 режима розыгрыша (drag-and-drop):
...
#### Структура хода:
1. **Враг раскрывает Intent** — игрок видит telegraph (иконка + режим)
2. **Фаза игрока** — drag-and-drop карты из руки в зоны
3. **Fate card** — после Strike/Influence тянется карта судьбы (engine-owned RNG)
4. **Фаза врага** — враг выполняет объявленный Intent
5. **Конец раунда** — проверка victory/defeat, draw hand
...
| `keyword` | Disposition-зависимый эффект (surge/focus/echo/shadow/ward) |
...
#### Momentum (инерция стратегии):
Последовательные действия одного типа усиливаются (streak bonus), но переключение штрафуется (switch penalty при streak ≥ 3). Это создаёт дилемму: дожать или сменить подход.

### 9.3 Формула effective_power
...
effective_power = min(raw_power, 25)   // hard cap: quarter of scale
...
**Hard cap 25** гарантирует, что никакая комбинация не позволяет закончить бой за 4 хода.
...
> В Disposition Combat основной вклад карт в исход боя — через режим игры (Strike / Influence / Sacrifice, см. §9.2).
> Эффекты ниже — дополнительные бонусы карты, применяемые сразу при игре.

| Эффект | Описание | Пример |
|--------|----------|--------|
| **damage(amount, type)** | Прямой урон герою (от врага) или элементальный урон | Огненный шар: 6 урона |
...
| **dispositionShift(amount)** | Прямой сдвиг disposition (поверх Strike/Influence) | Слово Силы: disposition +4 |
...
> **Deprecated (v3.2 → v4.0):** `bodyDamage` и `spiritDamage` заменены единой disposition-шкалой.
> Старый `sacrifice(cost, benefit)` как эффект карты заменён режимом Sacrifice (drag → костёр).
...
### 15.5 Баланс Destroy vs Subjugate (Disposition Combat)
- **Destroy (disposition → -100):** быстрее через Strike-streak, больше лута/трофеев, но резонанс → Навь, мир «грубеет»
- **Subjugate (disposition → +100):** дольше через Influence, требует больше ресурсов, но Правь усиливается, открываются другие ветки
- Оба пути жизнеспособны; «чистая» стратегия (только Destroy или только Subjugate) на всю кампанию — экстремальная, но возможная
- **Momentum system** создаёт trade-off: streak bonus поощряет специализацию, switch penalty наказывает метания, threat bonus поощряет стратегическое переключение
...
**Статус:** В разработке (synced with Disposition Combat v2.5, COMBAT_DIPLOMACY_SPEC v2.0)
