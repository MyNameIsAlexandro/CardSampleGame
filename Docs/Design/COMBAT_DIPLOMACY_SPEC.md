...
**Статус:** Implemented (Disposition Combat v2.5 + UX overhaul)
**Дата:** 3 марта 2026
...
| **Ввод игрока** | Кнопки Атака/Влияние/Ожидание | Tap-tap: карта + action button (Strike/Influence/Sacrifice), drag как альтернатива |
...
## 2. Card Play — 3 режима через tap-tap (drag как альтернатива)

### Основной ввод: tap-tap с action buttons

1. Игрок тапает карту в руке — карта выделяется, появляются 3 кнопки действий
2. Каждая кнопка показывает preview числа (рассчитанные через `DispositionCalculator.previewStrikePower/previewInfluencePower`)
3. Игрок тапает кнопку — карта летит к disposition bar (анимация 0.3s), действие применяется
4. Числа подсвечены: зелёный = boosted (выше базовой силы), красный = penalized

### Альтернативный ввод: drag

Drag карты к кнопке действия работает как альтернатива tap-tap, вызывает тот же canonical API.

| Действие | Tap-tap | Drag (alt) | Эффект | Карта после |
|----------|---------|------------|--------|-------------|
| **Strike** | Tap card → Tap Strike button | Drag → Strike area | disposition -= effective_power | Discard |
| **Influence** | Tap card → Tap Influence button | Drag → Influence area | disposition += effective_power | Discard |
| **Sacrifice** | Tap card → Tap Sacrifice button | Drag → Sacrifice area | Heal hero + enemy buff | Exhaust (навсегда) |

### Визуальная обратная связь

- **Card-to-bar fly animation** (0.3s): карта летит к disposition bar при успешном действии
- **Floating numbers**: красный для Strike (отрицательный shift), синий для Influence (положительный shift)
- **Fate keyword flash** (1s): центральная надпись при срабатывании Surge/Shadow/Ward/Focus/Echo
- **Enemy anticipation pulse**: пульсация идола перед действием врага
- **Enemy mode change flash**: текстовое уведомление при смене режима (Survival/Desperation/Weakened)
- **Hero HP damage float**: красное число у HP при получении урона
- **Post-combat summary**: оверлей с итогами (outcome, ходы, карты, disposition, HP) и кнопка "Далее"
...
**Дата:** 3 марта 2026
**Статус:** Implemented (UX overhaul complete, 148 gate tests pass)
