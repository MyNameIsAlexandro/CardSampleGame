...
- Используем `WorldRNG` (seeded instance) через `EngineServices.rng` для всех случайных операций
...
- `rng.setSeed(seed)` или `WorldRNG(seed: seed)` перед симуляцией обеспечивает воспроизводимость
