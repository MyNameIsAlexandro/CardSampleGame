...
**Версия:** 2.4 (Content Bible добавлен)
**Дата:** 16 января 2026
...
> **Статус реализации:** См. [TECHNICAL_DOCUMENTATION.md](./TECHNICAL_DOCUMENTATION.md) и [CAMPAIGN_IMPLEMENTATION_REPORT.md](./CAMPAIGN_IMPLEMENTATION_REPORT.md)

| # | Требование |
|---|-----------|
| 1 | Время увеличивается при действиях игрока |
| 2 | WorldTension растёт автоматически каждые 3 дня |
| 3 | Регионы деградируют по весовому алгоритму (Borderland:1, Breach:2) |
| 4 | Якоря сопротивляются деградации (integrity > 50% → пропуск) |
| 5 | Выбор региона показывает риск и награды |
| 6 | События выбираются из пулов по RegionState |
| 7 | Бой запускается только через события (не напрямую) |
| 8 | Последствия событий записываются через флаги |
| 9 | Квесты проверяют флаги для продвижения |
| 10 | WorldTension = 100% → Game Over |
...
> **Каноническая версия Definition of Done** находится в [EXPLORATION_CORE_DESIGN.md, раздел 18.10](./EXPLORATION_CORE_DESIGN.md#1810-definition-of-done-core-loop)
