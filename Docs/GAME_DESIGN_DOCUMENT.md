...
**Автоматический рост:** Каждые 3 дня `worldTension += 3`
...
### 7.0 Система классов героев (HeroClass)

Каждый герой принадлежит к определённому классу, который определяет:
- **Базовые характеристики** (HP, сила, вера, интеллект и др.)
- **Особую способность** (уникальный боевой бонус)
- **Стартовый путь** (Light/Dark/Balance)

#### Таблица классов

| Класс | HP | Сила | Вера | Особая способность |
|-------|-----|------|------|-------------------|
| **Воин** | 12 | 7 | 2 | Ярость: +2 к урону при HP < 50% |
| **Маг** | 7 | 2 | 5 | Медитация: +1 вера в конце хода |
| **Следопыт** | 10 | 4 | 3 | Выслеживание: +1 кубик при первой атаке |
| **Жрец** | 9 | 3 | 5 | Благословение: -1 урон от тёмных источников |
| **Тень** | 8 | 4 | 4 | Засада: +3 урона по целям с полным HP |

#### Стартовые пути

- **Воин, Маг, Следопыт** → Balance (нейтральный баланс 50)
- **Жрец** → Light (баланс 70, склонен к Свету)
- **Тень** → Dark (баланс 30, склонен к Тьме)
...
```
Атака = Сила героя + d6 + бонусные_кубики + бонусный_урон
...
### 9.4 Эффекты карт в бою (AbilityEffect)
