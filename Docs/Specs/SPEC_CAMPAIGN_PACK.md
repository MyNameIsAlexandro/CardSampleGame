...
| FR-ENM-006 | Враг МОЖЕТ иметь Волю (Will) как альтернативное условие победы | Опционально |
| FR-ENM-007 | Враг МОЖЕТ иметь повторяющийся паттерн поведения | Опционально |
| FR-ENM-008 | Враг МОЖЕТ иметь модификаторы от резонанса | Опционально |
...
```json
{
  ...
  "weaknesses": "string[] | null (опционально, ключевые уязвимости)",
  "strengths": "string[] | null (опционально, ключевые сопротивления)"
}
```
...
```json
{
  ...
  "value": "integer (значение эффекта, например урон атаки или количество лечения)"
}
```

Паттерн определяет циклическую последовательность действий врага. Враг выбирает шаг по формуле `pattern[(round - 1) % pattern.length]`. Если паттерн не задан, AI генерирует намерение случайно.

#### EnemyModifier (Resonance)

```json
{
  "power_delta": "integer (модификатор силы)",
  "defense_delta": "integer (модификатор защиты)",
  "health_delta": "integer (модификатор здоровья)",
  "will_delta": "integer (модификатор воли)"
}
```
...
| 1.1 | 2026-02-01 | Claude | Добавлены will, pattern, resonance_behavior, weaknesses, strengths для врагов |
