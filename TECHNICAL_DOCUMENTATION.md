...
1. **🎯 Интеграция боевой системы с событиями**
   ...
   - Передача монстра в бой (уже есть monsterCard в событиях)
   - Обработка победы/поражения
   - Применение наград/штрафов

2. **🎨 UI обновления**
   - Отображение шкалы баланса 0-100 (вместо -10/+10)
   - Визуализация квестов в RegionDetailView
   - Индикатор времени и деградации
   - Отображение completed objectives

3. **🃏 Карты-награды из событий**
   - Создать карты, упомянутые в событиях:
...
4. **🌍 Расширение контента Акта I**
   - Добавить босса Леший-Хранитель
   - Создать уникальные события для каждого региона
   - Добавить артефакты (guardian_seal, ancient_relic)

5. **📜 Квестовые триггеры и прогресс**
   - Автоматическое обновление objectives
   - Триггеры квестов из событий (questLinks)
   - UI для отслеживания прогресса
   - Система выдачи наград

6. **⚔️ Расширение боевой системы**
   - Добавить способности монстров
   - Босс-механики (Леший-Хранитель)
   - Награды за победу над элитами
